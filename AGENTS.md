...
以下规则优先级等同安全边界，不能用“临时方案”“先占位”“后面再接真实数据”等理由绕过。
...
- 用户提供 UI 设计稿、截图、Figma、原型或参考图时，UI 开发完成后必须逐屏对比实现结果和用户提供的 UI。
- 对比内容至少包括布局、间距、颜色、字体层级、组件状态、交互入口、空/加载/错误态。
- 最终交付必须说明一致项、差异项和未对齐原因；未经对比不得宣称 UI 完成。

### 注释和可维护性

- 新增或修改代码必须补充必要的简体中文注释，尤其是 Go 后端、Rust 桌面能力、安全链路、数据库事务、任务状态机、Provider、凭据和脱敏逻辑。
- 注释要解释业务意图、数据流、安全边界、异常处理和关键取舍；不要写“把 A 赋值给 B”这类无信息量注释。
- 公共函数、跨模块契约、复杂流程和容易误用的边界必须有中文说明。
- Rust 和 Go 的所有函数都必须有简体中文注释；测试函数也必须说明验证的业务边界或风险点。
- Go 的每个包都必须提供 `doc.go`，用包注释说明该包的职责、边界和不应该承载的逻辑。

### 设计克制和第三方优先

- 避免过度设计，不能为了假想未来需求提前抽象、提前扩展或提前引入复杂配置。
- 没有稳定复用场景时，不创建 helper、factory、generic abstraction、插件化接口或多层 wrapper。
- 只要存在活跃、成熟、维护良好的第三方开源组件或官方能力可以满足需求，禁止自研同类基础能力。
- 禁止自研 UI 基础组件、图表引擎、Markdown 安全解析、日期时间处理、表单校验、HTTP/SSE 基础设施、数据库迁移/查询生成等已有成熟方案，除非已有组件无法满足明确的安全、许可或跨平台约束，并在实施前说明理由。
- 引入第三方依赖必须仍遵守项目确认规则：新增、升级、移除依赖或修改锁文件前需要明确确认。
...
- checklist 只勾选已验证任务。
- 文档与代码行为一致。
- UI 完成项已和用户提供的 UI 做对比，并记录差异。
- 没有 mock 数据、假按钮、散落数据源或重复权威状态。
- 关键后端、安全、数据流代码已有必要中文注释。
- 没有在成熟第三方开源组件可用时自研基础能力。
- `git diff --check` 通过。
