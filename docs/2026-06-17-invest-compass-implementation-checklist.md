...
- 当前进展：
  ...
  - 受 T09/T14 后续接入约束，数据库持久化、`/api/watchlist/*` 和 Rust command 完成后再标记为 `[x]`。
...
- 当前进展：
  ...
  - 受 T11/T20/T30 依赖约束，真实 settings/cache/provider API、系统凭据写入、Rust command 和设置页接入后再标记为 `[x]`。
...
- 当前进展：
  ...
  - 后续仍需随 T08/T09/T20/T42/T44 等真实实现和跨平台验收结果继续更新。
