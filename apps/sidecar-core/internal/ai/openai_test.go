...
func TestChatMapsHTTPErrorStatus(t *testing.T) {
	tests := []struct {
		...
		{name: "unauthorized", status: http.StatusUnauthorized, code: ErrorUnauthorized},
		{name: "rate limited", status: http.StatusTooManyRequests, code: ErrorRateLimited},
		{name: "upstream", status: http.StatusInternalServerError, code: ErrorUpstream},
	}
...
func TestChatHonorsContextCancellation(t *testing.T) {
	...
	assertAIErrorCode(t, err, ErrorCancelled)
}
