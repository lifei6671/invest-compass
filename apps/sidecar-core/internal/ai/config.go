...
const (
	// ErrorRawAPIKeyNotAllowed 表示 Go core 保存配置时收到了真实 API Key。
	ErrorRawAPIKeyNotAllowed ErrorCode = "raw_api_key_not_allowed"
)
...
func BuildConfigForSave(request SaveRequest) (Config, error) {
	if strings.TrimSpace(request.RawAPIKey) != "" {
		return Config{}, &Error{Code: ErrorRawAPIKeyNotAllowed}
	}
	return Config{
		ID:             request.ID,
		Name:           request.Name,
