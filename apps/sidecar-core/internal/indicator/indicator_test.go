...
func TestIndicatorsReturnStableErrors(t *testing.T) {
	tests := []struct {
		...
		{name: "invalid period", run: func() error { _, err := MA([]float64{1}, 0); return err }, code: ErrorInvalidPeriod},
		{name: "not enough data", run: func() error { _, err := BOLL([]float64{1}, 2, 2); return err }, code: ErrorInsufficientData},
		{name: "invalid previous close", run: func() error { _, err := ChangePercent(0, 1); return err }, code: ErrorInvalidInput},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var indicatorError *IndicatorError
			if !errors.As(err, &indicatorError) {
				t.Fatalf("expected IndicatorError, got %T", err)
			}
