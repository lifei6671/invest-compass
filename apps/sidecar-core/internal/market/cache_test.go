...
func TestNewQuoteCacheValidatesShortTTL(t *testing.T) {
	tests := []struct {
		...
		{name: "too short", ttl: 9 * time.Second, code: ErrorInvalidQuoteCacheTTL},
		{name: "too long", ttl: 61 * time.Second, code: ErrorInvalidQuoteCacheTTL},
	}
...
func assertMarketErrorCode(t *testing.T, err error, code ErrorCode) {
	t.Helper()
	if err == nil {
