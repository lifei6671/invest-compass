...
// ErrorCode 是行情模块对外稳定的错误码。
type ErrorCode string

const (
	// ErrorInvalidQuoteCacheTTL 表示行情短缓存 TTL 不在 10-60 秒范围内。
	ErrorInvalidQuoteCacheTTL ErrorCode = "invalid_quote_cache_ttl"
)
...
// Error 返回稳定错误码字符串，避免把 Provider 原始错误拼进规则错误。
...
func NewQuoteCache(ttl time.Duration) (*QuoteCache, error) {
	if ttl < minQuoteCacheTTL || ttl > maxQuoteCacheTTL {
		return nil, &Error{Code: ErrorInvalidQuoteCacheTTL}
	}
	return &QuoteCache{
		ttl:     ttl,
