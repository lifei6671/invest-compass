...
func TestCreateItemRejectsDuplicateActiveSymbol(t *testing.T) {
	...
	assertWatchlistErrorCode(t, err, ErrorDuplicateActiveSymbol)
}
...
func assertWatchlistErrorCode(t *testing.T, err error, code ErrorCode) {
	t.Helper()
	if err == nil {
	...
	if !errors.As(err, &watchlistError) {
		t.Fatalf("expected watchlist Error, got %T", err)
	}
