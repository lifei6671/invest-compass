...
const (
	// ErrorMissingTitle 表示新闻标题为空。
	ErrorMissingTitle Code = "missing_news_title"
	// ErrorUnsafeURL 表示新闻链接使用了不允许的 URL scheme。
	ErrorUnsafeURL Code = "unsafe_news_url"
)
...
// Error 返回稳定错误码字符串，避免把原始新闻内容写入错误文本。
...
func NormalizeItem(item Item) (Item, error) {
	...
	if item.Title == "" {
		return Item{}, &Error{Code: ErrorMissingTitle}
	}
...
func normalizeURL(raw string) (string, error) {
	...
	if err != nil || parsed.Scheme == "" {
		return "", &Error{Code: ErrorUnsafeURL}
	}
