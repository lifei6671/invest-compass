...
func MA(values []float64, period int) ([]float64, error) {
	...
	if len(values) < period {
		return nil, newIndicatorError(xerr.IndicatorInsufficientData)
	}
...
func MACD(values []float64, fastPeriod int, slowPeriod int, signalPeriod int) (MACDSeries, error) {
	if fastPeriod <= 0 || slowPeriod <= 0 || signalPeriod <= 0 || fastPeriod >= slowPeriod {
		return MACDSeries{}, newIndicatorError(xerr.IndicatorInvalidPeriod)
	}
...
func KDJ(klines []KLine, period int) (KDJSeries, error) {
	...
	if len(klines) < period {
		return KDJSeries{}, newIndicatorError(xerr.IndicatorInsufficientData)
	}
...
func BOLL(values []float64, period int, multiplier float64) (BOLLSeries, error) {
	...
	if len(values) < period {
		return BOLLSeries{}, newIndicatorError(xerr.IndicatorInsufficientData)
	}
	if multiplier <= 0 {
		return BOLLSeries{}, newIndicatorError(xerr.IndicatorInvalidInput)
	}
...
func MaxDrawdown(values []float64) (float64, error) {
	if len(values) == 0 {
		return 0, newIndicatorError(xerr.IndicatorInsufficientData)
	}
	peak := values[0]
	maxDrawdown := 0.0
...
func requirePeriod(period int) error {
	if period <= 0 {
		return newIndicatorError(xerr.IndicatorInvalidPeriod)
	}
	return nil
}

// newIndicatorError 创建稳定错误码错误，避免把原始行情数据拼进错误文本。
func newIndicatorError(code xerr.Code) error {
	return &xerr.Error{Code: code}
}
