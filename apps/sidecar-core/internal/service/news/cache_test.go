...
func TestNewCacheValidatesTTL(t *testing.T) {
	tests := []struct {
		...
		{name: "too short", ttl: 29 * time.Minute, code: xerr.NewsInvalidCacheTTL},
		{name: "too long", ttl: 121 * time.Minute, code: xerr.NewsInvalidCacheTTL},
	}
...
func assertNewsErrorCode(t *testing.T, err error, code xerr.Code) {
	t.Helper()
	if err == nil {
		t.Fatal("expected error")
	}

	newsError, ok := err.(*xerr.Error)
	if !ok {
