...
export function MiniTrendChart(props: MiniTrendChartProps) {
  ...
  if (points.length === 0) {
    return (
      <div className="flex h-12 w-28 items-center justify-center rounded-md border border-dashed border-[#dbe5f2] bg-[#f8fafc] text-[11px] text-[#8a94a6]">
        无走势数据
      </div>
    );
  }
  ...
  const linePoints = points
    .map((point, index) => {
      const x = (index / Math.max(points.length - 1, 1)) * 108 + 2;
  ...
  return (
    <svg aria-label={props.trend === "up" ? "上涨走势" : "下跌走势"} className="h-12 w-28" viewBox="0 0 112 40" role="img">
      <defs>
